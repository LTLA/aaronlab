...
These are BAM files, so we've already mapped the reads to the mm10 genome.
...
Windows of constant size are tiled across the genome at regular intervals.
...
data <- windowCounts(bam.files, ext=frag.len, width=window.width, param=param)
data
```
...
This is introduced when DB regions take up a bigger slice of the sequencing pie in one library, suppressing the coverage of everything else like background regions.
...
First, we count reads into large bins to quantify the coverage of the background regions.

```{r}
binned <- windowCounts(bam.files, bin=TRUE, width=10000, param=param)
```

Any differences between libraries across these background bins must be artifactual, and scaling is performed to eliminate them.
...
Filtering gets rid of most of them, while also reducing computational work and the severity of the multiple testing correction.
...
We keep those windows that have a 3-fold or higher increase in abundance.
...
We fit a generalized linear model (GLM) with estimates for the QL dispersions. 
...
We also get some other statistics, like the total number of windows in each cluster as well as the number that are changing in each direction.
...
Controlling the FDR across regions is simple, as we can just apply the correction on the combined *p*-values.
...
We can use a whole lot of different clustering strategies. 
One that is particularly popular is to cluster windows according to the promoters in which they are assigned.
...
```{r}
anno2 <- detailRanges(clustered$regions, orgdb=org.Mm.eg.db,
  txdb=TxDb.Mmusculus.USCS.mm10.knownGene, dist=3000, promoter=c(0, 2000))
```
